...
#define JNI_FN(A) Java_com_ibexreader_mibooksi_ ## A
#define PACKAGENAME "com/ibexreader/mibooksi"

#define LOG_TAG "libmibook"
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addMarkupAnnotationInternal)(JNIEnv * env, jobject thiz, jobjectArray points, fz_annot_type type, jfloatArray rgba)
{
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	...
	int col_num = (*env)->GetArrayLength(env,rgba);
	jfloat *col = (*env)->GetFloatArrayElements(env,rgba,0);
	for (i = 0; i < col_num-1; i++)
	{

		color[i] = col[i];
	}
	float alpha = col[col_num-1];

	(*env)->ReleaseFloatArrayElements(env,rgba, col,0);
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addInkAnnotationInternal)(JNIEnv * env, jobject thiz, jobjectArray arcs, jfloatArray rgb, jfloat inkWidth)
{
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	...
	int col_num = (*env)->GetArrayLength(env,rgb);
	jfloat *col = (*env)->GetFloatArrayElements(env,rgb,0);
	for (i = 0; i < col_num; i++)
	{
	...
	(*env)->ReleaseFloatArrayElements(env,rgb, col,0);
	// color[0] = rgb[0];
	// color[1] = rgb[1];
	// color[2] = rgb[2];
	...
	fz_try(ctx)
	{
		...
		pdf_set_ink_annot_list(ctx, idoc, (pdf_annot *)annot, pts, counts, n, color, inkWidth);
