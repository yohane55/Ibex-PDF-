...
import com.ibexreader.mibooksi.views.ColorPicker;
...
{
	private View mAnnotColorView;
	private ColorPicker mColorPicker;
	private Uri uri;
	private BottomBar mBottumBar;

	/* The core rendering instance */
	enum TopBarMode {Main, Search,  Accept, Delete,};
	...
	public void createUI(Bundle savedInstanceState) {
		...
		mBottumBar = BottomBar.attach(this, savedInstanceState);
		mBottumBar.setItems(R.menu.toolbar_menu);
		mBottumBar.noTopOffset();




		mBottumBar.mapColorForTab(0,getResources().getColor(R.color.accent));
		mBottumBar.mapColorForTab(1,getResources().getColor(R.color.md_purple_700));
		mBottumBar.mapColorForTab(2,getResources().getColor(R.color.md_pink_700));
		mBottumBar.mapColorForTab(3,getResources().getColor(R.color.md_blue_700));
		mBottumBar.mapColorForTab(4,getResources().getColor(R.color.md_lime_700));
		mBottumBar.mapColorForTab(5,getResources().getColor(R.color.md_cyan_700));
		mBottumBar.mapColorForTab(6,getResources().getColor(R.color.md_indigo_600));
		mBottumBar.mapColorForTab(7,getResources().getColor(R.color.md_deep_orange_700));
		final BottomBarBadge unreadMessages = mBottumBar.makeBadgeForTabAt(2, "#FFFF00", 1);

		mBottumBar.setOnMenuTabClickListener(new OnMenuTabClickListener() {
			@Override
			public void onMenuTabSelected(@IdRes int menuItemId) {
				switch (menuItemId){
					case R.id.read_noaction:

						if(mDocView!=null) {
							OnCancelAcceptButtonClick(null);
							OnCancelAnnotButtonClick(null);
							OnCancelDeleteButtonClick(null);
							OnCancelSearchButtonClick(null);
...
//						}
					...
					case R.id.searchButton:
						if(mReflow){
							showInfo("Not available in reflow mode");
							break;
						}
		...
		FrameLayout layout = (FrameLayout) findViewById(R.id.reader_view);
		...
		mAnnotColorView.setOnClickListener(new View.OnClickListener() {
			@Override
...
//			mAnnotButton.setVisibility(View.GONE);
	...
	private void showPopUp() {
		...
		if(mColorPicker == null){
			...
			mColorPicker.setOnColorChanged(new ColorPicker.OnColorChanged() {
				@Override
				public void onColorChanged(int color, float width) {
					mAnnotColorView.setBackgroundColor(color);

					if(mAnnotTypeText.getText().toString().contains("Draw")) {
						core.setInkAnnotColor(getRGB(color));
						core.setInkWidth(width);


						MiBooksApplication.getInstance().setInkColor(getRGB(color));
						MiBooksApplication.getInstance().setInkWidth(width *
						2);
					}else if(mAnnotTypeText.getText().toString().contains("Underline")) {
						core.setMarkUpAnnotColor(getRGBA(color, 0.3f));
						MiBooksApplication.getInstance().setHighlightColor(getRGBA(color, 1f));

					}else{
						final float[] f = getRGBA(color, 0.3f);

						core.setMarkUpAnnotColor(f);
						core.setInkAnnotColor(getRGB(color));
						core.setInkWidth(width);
						MiBooksApplication.getInstance().setHighlightColor(f);
					}
			...
			mColorPicker.setWindowLayoutMode(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
		}


		mColorPicker.setSeekBarValue(MiBooksApplication.getInstance().getInkWidth());


		mColorPicker.showAsDropDown(mAnnotColorView);
//		mColorPicker.update(mAnnotColorView, ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
	}


	public float[] getRGB(int color){
		final float[] f = new float[3];
		f[0]=Color.red(color)/255.0f;
		f[1]=Color.green(color)/255.0f;
		f[2]=Color.blue(color)/255.0f;

		return f;
	}

	public int makeColor(float[] rgb){


		return Color.rgb((int)(rgb[0]*255),(int)(rgb[1]*255),(int)(rgb[2]*255));
	}
	public float[] getRGBA(int color, float v /*
	alpha*/){
...
//		setButtonEnabled(mAnnotButton, !reflow);
	...
	protected void onSaveInstanceState(Bundle outState) {
		...
		mBottumBar.onSaveInstanceState(outState);
	}
	...
	private void makeButtonsView() {
		...
		mAnnotColorView = (View)findViewById(R.id.annotColor);
		...
		mReflowButton = (ImageButton)findViewById(R.id.reflowButton);
		mOutlineButton = (ImageButton)findViewById(R.id.outlineButton);
//		mAnnotButton = (ImageButton)findViewById(R.id.editAnnotButton);
		...
		mProofButton = (ImageButton)findViewById(R.id.proofButton);
		mSepsButton = (ImageButton)findViewById(R.id.sepsButton);
...
//					break;
	...
	public void OnCopyTextButtonClick(View v) {
		...
		mAnnotColorView.setVisibility(View.GONE);
	...
	public void OnEditAnnotButtonClick(View v) {
		mTopBarMode = TopBarMode.Main;
		mAnnotColorView.setVisibility(View.VISIBLE);
		mTopBarSwitcher.setDisplayedChild(mTopBarMode.ordinal());
	}
	...
	public void OnHighlightButtonClick(View v) {
		mTopBarMode = TopBarMode.Accept;
		MiBooksApplication.getInstance().setHighlightColor(getRGBA(Color.argb(100,250,250,0), 0.3f));

		mTopBarSwitcher.setDisplayedChild(mTopBarMode.ordinal());
		showButtons();
		mAcceptMode = AcceptMode.Highlight;
		mAnnotColorView.setVisibility(View.VISIBLE);
		mAnnotColorView.setBackgroundColor(makeColor(MiBooksApplication.getInstance().getHighlightColor()));
		mDocView.setMode(MuPDFReaderView.Mode.Selecting);
	...
	public void OnUnderlineButtonClick(View v) {
		...
		MiBooksApplication.getInstance().setHighlightColor(getRGBA(Color.BLACK, 0.3f));
		mAnnotColorView.setBackgroundColor(Color.BLACK);
		mTopBarMode = TopBarMode.Accept;
		mTopBarSwitcher.setDisplayedChild(mTopBarMode.ordinal());
		showButtons();
...
//		mAnnotColorView.setBackgroundColor(makeColor(MiBooksApplication.getInstance().getHighlightColor()));
	...
	public void OnInkButtonClick(View v) {
		...
		showButtons();
		mAnnotColorView.setVisibility(View.VISIBLE);
		mAnnotColorView.setBackgroundColor(makeColor(MiBooksApplication.getInstance().getInkColor()));
	...
	public void OnAcceptButtonClick(View v) {
		...
		mBottumBar.selectTabAtPosition(0, true);
//		mDocView.setMode(MuPDFReaderView.Mode.Viewing);
	}
	...
	public void onBackPressed() {
		...
		else if (core != null && core.hasChanges()) {
			DialogInterface.OnClickListener listener = new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
	...
	private void deleteThumb(Uri uri) {
		String path = Uri.decode(uri.getEncodedPath());
		if (path == null) {
			path = uri.toString();
		}
		...
		File thumbdir = new File(parent,".thumbs");

			File thumbfile =  new File(thumbdir.getPath(),f.getName()+".thumb");
			if(thumbfile.delete()){
				Log.d("Mibooks", "deleteThumb: "+"thumbnail deleted :"+thumbfile.getPath());
			}
