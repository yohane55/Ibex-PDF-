...
public class MuPDFPageView extends PageView implements MuPDFView {
	...
	public boolean hitonDeleteButton(float x, float y){
		if(mSelectedAnnotationIndex>-1) {
			RectF ann = mAnnotations[mSelectedAnnotationIndex];

			if(ann!=null) {
				float rad = dpToPx(13, getResources()) * getScale();

				final RectF del = new RectF();

				del.left = ann.right - rad;
				del.bottom = ann.top + rad;
				del.right = ann.right + rad;
				del.top = ann.top - rad;


				return del.contains(x* getScale(),y* getScale());
			}
	...
	public boolean copySelection() {
		...
		if (currentApiVersion >= android.os.Build.VERSION_CODES.HONEYCOMB) {
			...
			cm.setPrimaryClip(ClipData.newPlainText(getResources().getString(R.string.app_name), text));
		} else {
	...
	public boolean saveDraw() {
		...
		mAddInk = new AsyncTask<PointF[][],Void,Void>() {
			@Override
			protected Void doInBackground(PointF[][]... params) {
				mCore.addInkAnnotation(mPageNumber, params[0],null);
				return null;
			}
	...
	protected void addMarkup(PointF[] quadPoints, Annotation.Type type) {
		mCore.addMarkupAnnotation(mPageNumber, quadPoints, type, null);
	}
