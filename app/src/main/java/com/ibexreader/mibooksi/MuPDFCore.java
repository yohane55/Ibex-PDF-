...
public class MuPDFCore
{
	...
	public void setMarkUpAnnotColor(float[] markUpAnnotColor) {
		this.markUpAnnotColor = markUpAnnotColor;
	}
	...
	public void setInkAnnotColor(float[] inkAnnotColor) {
		this.inkAnnotColor = inkAnnotColor;
	}

	private float[] markUpAnnotColor = {1,1,0,0.5f};
	private float[] inkAnnotColor ={1,0,0};
	...
	private native void addMarkupAnnotationInternal(PointF[] quadPoints, int type, float[] argb);
	private native void addInkAnnotationInternal(PointF[][] arcs,float[] rgb, float width);
	...
	public synchronized void addMarkupAnnotation(int page, PointF[] quadPoints, Annotation.Type type, float[] argb) {
		gotoPage(page);

		if(argb!=null && argb.length>=4){
			markUpAnnotColor = argb;
		}
		addMarkupAnnotationInternal(quadPoints, type.ordinal(), MiBooksApplication.getInstance().getHighlightColor());
	}

	public synchronized void addInkAnnotation(int page, PointF[][] arcs, float[] rgb) {
		gotoPage(page);
		if(rgb!=null && rgb.length>=3){
			inkAnnotColor =rgb;
		}
		addInkAnnotationInternal(arcs, MiBooksApplication.getInstance().getInkColor(),  MiBooksApplication.getInstance().getInkWidth()-2);
	}
