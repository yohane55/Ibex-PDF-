...
public abstract class PageView extends ViewGroup {
	...
	private static float INK_THICKNESS = 5.0f;
	private Bitmap del;
	private float scale=1;

	public static void setInkColor(int inkColor) {
		INK_COLOR = inkColor;
	}

	public static void setInkThickness(float inkThickness) {
			...
			mSearchView = new View(mContext) {
				...
				protected void onDraw(final Canvas canvas) {
					...
					if (mItemSelectBox != null) {
						...
						paint.setStrokeWidth(dpToPx(2,getResources()));
						...
						final int radius = 4; //dp
						{

							paint.setStyle(Paint.Style.STROKE);
							paint.setColor(Color.BLACK);
							canvas.drawCircle(mItemSelectBox.right * scale, mItemSelectBox.top * scale, dpToPx(radius+1, getResources()) * scale, paint);
							canvas.drawCircle(mItemSelectBox.left * scale, mItemSelectBox.top * scale, dpToPx(radius+1, getResources()) * scale, paint);
							canvas.drawCircle(mItemSelectBox.right * scale, mItemSelectBox.bottom * scale, dpToPx(radius+1, getResources()) * scale, paint);
							canvas.drawCircle(mItemSelectBox.left * scale, mItemSelectBox.bottom * scale, dpToPx(radius+1, getResources()) * scale, paint);


							paint.setStyle(Paint.Style.FILL_AND_STROKE);
							paint.setColor(Color.WHITE);
							canvas.drawCircle(mItemSelectBox.right * scale, mItemSelectBox.top * scale, dpToPx(radius, getResources()) * scale, paint);
							canvas.drawCircle(mItemSelectBox.left * scale, mItemSelectBox.top * scale, dpToPx(radius, getResources()) * scale, paint);
							canvas.drawCircle(mItemSelectBox.right * scale, mItemSelectBox.bottom * scale, dpToPx(radius, getResources()) * scale, paint);
							canvas.drawCircle(mItemSelectBox.left * scale, mItemSelectBox.bottom * scale, dpToPx(radius, getResources()) * scale, paint);

						}
//						paint.setStyle(Paint.Style.STROKE);
//						paint.setColor(Color.WHITE);

//						canvas.drawCircle(mItemSelectBox.right * scale, mItemSelectBox.top * scale, dpToPx(13, getResources()) * scale, paint);
...
//
//						paint.setColor(Color.WHITE);
//						paint.setTextSize(dpToPx(10, getResources()) * scale);
//						if (del == null) {
//							BitmapFactory.Options opts = new BitmapFactory.Options();
////							if(scale>1)opts.inSampleSize = 1;
////							else opts.inSampleSize = 2;
//							del = BitmapFactory.decodeResource(getResources(), R.drawable.ic_delete, opts);
//						}
//						canvas.drawBitmap(del, (mItemSelectBox.right * scale) - del.getWidth() / 2,
//								(mItemSelectBox.top * scale) - del.getHeight() / 2, paint);
					...
					if (mDrawing != null) {
						...
						paint.setStyle(Paint.Style.FILL);
						paint.setStrokeWidth(MiBooksApplication.getInstance().getInkWidth() * scale);
						paint.setColor(makeColor(MiBooksApplication.getInstance().getInkColor()));
						...
						while (it.hasNext()) {
							...
							if (arc.size() >= 2) {
								...
								p = arc.get(0);
								canvas.drawCircle(p.x * scale, p.y * scale, MiBooksApplication.getInstance().getInkWidth() * scale / 2, paint);
							}
						}
	...
	private int makeColor(float[] inkColor) {
		if(inkColor==null || inkColor.length<3){
			return Color.BLUE;
		}
		return Color.rgb((int)(inkColor[0]*255),(int)(inkColor[1]*255),(int)(inkColor[2]*255));
	}
